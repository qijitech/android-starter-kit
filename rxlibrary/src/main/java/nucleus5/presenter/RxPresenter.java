...
public class RxPresenter<View> extends Presenter<View> {
   ...
   * @param observableFactory a factory that should return an Observable when the restartable
   * should
   * run.
  ...
  /**
   * This is a shortcut for calling {@link #restartableFirst(int, Factory, BiConsumer, BiConsumer)}
  ...
  /**
   * This is a shortcut for calling {@link #restartableLatestCache(int, Factory, BiConsumer, *
   * BiConsumer)} with the last parameter = null.
   */
  public <T> void restartableLatestCache(int restartableId,
      final Factory<Observable<T>> observableFactory, final BiConsumer<View, T> onNext) {
  ...
  /**
   * This is a shortcut for calling {@link #restartableReplay(int, Factory, BiConsumer, BiConsumer)}
   * with the last parameter = null.
   */
  public <T> void restartableReplay(int restartableId,
      final Factory<Observable<T>> observableFactory, final BiConsumer<View, T> onNext) {
  ...
  /**
   * Returns an {@link io.reactivex.ObservableTransformer} that couples views with data that has
   * been emitted by
   * the source {@link io.reactivex.Observable}.
   * <p>
   * {@link #deliverLatestCache} keeps the latest onNext value and emits it each time a new view
   * gets attached.
   ...
   * the source {@link io.reactivex.Observable}.
   * <p>
   * {@link #deliverFirst} delivers only the first onNext value that has been emitted by the source
   * observable.
   *
   * @param <T> the type of source observable emissions
   */
   ...
   * the source {@link io.reactivex.Observable}.
   * <p>
   * {@link #deliverReplay} keeps all onNext values and emits them each time a new view gets
   * attached.
   * If a new onNext value appears while a view is attached, it will be delivered immediately.
   *
   * @param <T> the type of source observable emissions
   ...
   * @return an Action1 that splits a received {@link Delivery} into two {@link BiConsumer} onNext
   * and onError calls.
   */
  ...
  /**
   * This is a shortcut for calling {@link #split(BiConsumer, BiConsumer)} when the second parameter
   * is null.
   */
   ...
   * {@inheritDoc}
   */
  @CallSuper @Override protected void onCreate(Bundle savedState) {
    if (savedState != null) {
   ...
   * {@inheritDoc}
   */
  @CallSuper @Override protected void onSave(Bundle state) {
   ...
   * {@inheritDoc}
   */
  @CallSuper @Override protected void onTakeView(View view) {
   ...
   * {@inheritDoc}
   */
  @CallSuper @Override protected void onDropView() {
   ...
   */
  @Deprecated @Nullable @Override public View getView() {
