    ...
    @Override public ObservableSource<Delivery<View, T>> apply(Observable<T> observable) {
        return observable.materialize()
            ...
            .switchMap(new Function<Notification<T>, ObservableSource<Delivery<View, T>>>() {
              @Override
              public ObservableSource<Delivery<View, T>> apply(final Notification<T> notification)
                  throws Exception {
                return view.concatMap(
                    new Function<OptionalView<View>, ObservableSource<Delivery<View, T>>>() {
                      @Override
                      public ObservableSource<Delivery<View, T>> apply(OptionalView<View> view)
                          throws Exception {
