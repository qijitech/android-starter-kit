...
public class Presenter<View> {
   ...
   *
   * @param savedState If the presenter is being re-instantiated after a process restart then this
   * Bundle
  ...
  /**
   * A returned state is the state that will be passed to {@link #onCreate} for a new presenter
   * instance after a process restart.
   *
   * This method is intended for overriding.
   *
   ...
   * Normally this happens during {@link Activity#onResume()}, {@link
   ...
   * Notice here that {@link Activity#onActivityResult(int, int, Intent)} is called *before* {@link
   * Activity#onResume()}
